    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? t("common.loading") : t("common.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agents.config.name")} hint={help.name}>
              <DraftInput
      ...
      {forcedKubernetes ? (
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agents.config.environment")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agents.config.environment")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("agents.config.defaultEnvironment")}
              hint={t("agents.config.kubernetesHint")}
            >
              {kubernetesEnvironment ? (
                <div className={cn(inputClass, "flex items-center text-muted-foreground")}>
                  {kubernetesEnvironment.name} · {t("agents.config.kubernetesSandbox")}
                </div>
              ) : (
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
                  {t("agents.config.kubernetesMissing")}
                </div>
              )}
            </Field>
          </div>
        ...
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agents.config.environmentOverride")}>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("agents.config.workingDirectoryDeprecated")} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agents.config.permissionsConfiguration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agents.config.permissionsConfiguration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {supportsModelProfiles && (
                <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("agents.config.primaryModel")}</div>
              )}
              <ModelDropdown
                models={models}
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agents.config.bootstrapPrompt")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
              ...
              {!isCreate && (
                <Field label={t("agents.config.secretAccess")} hint={help.secretAccess}>
              ...
              {!isCreate && (
                <>
                  <Field label={t("agents.config.timeout")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agents.config.gracePeriod")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <div className="space-y-3">
              ...
              />
              <Field label={t("agents.config.cooldown")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("agents.config.continueAfterMaxTurn")}
                ...
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("agents.config.continuationAttempts")} hint={help.maxTurnContinuationMaxAttempts}>
                      <DraftNumberInput
                    ...
                    <Field label={t("agents.config.continuationDelay")} hint={help.maxTurnContinuationDelaySec}>
                      <DraftNumberInput
      ...
      <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-(length:--text-nano) text-muted-foreground">{t("agents.config.comingSoon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
        ...
        <PopoverContent className="w-(--radix-popover-trigger-width) p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("agents.config.searchModelsCreatable") : t("agents.config.searchModels")}
              value={modelSearch}
    ...
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-(length:--text-micro) uppercase tracking-wide text-muted-foreground">{t("agents.config.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">
