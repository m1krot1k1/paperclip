...
const FILTER_OPTIONS: Array<{ value: FilterValue; key: string }> = [
  { value: "all", key: "all" },
  { value: "in-progress", key: "inProgress" },
  { value: "for-review", key: "inReview" },
...
function CollapsedFeedGroup({
  ...
  const actorName = actor?.name
    ?? (group.latestEvent.actorType === "system" ? t("activity.system")
      : group.latestEvent.actorType === "user" ? t("activity.board")
      : t("activity.unknown"));
    ...
    <div>
      ...
      >
        ...
        <span className="flex-1 min-w-0 truncate">
          ...
          <span data-fc="verb" className="ml-1 text-(--hex-959596)">{t("activity.madeUpdates", { count: group.events.length })}</span>
...
export function ActivityFeed({ className }: ActivityFeedProps) {
        ...
        separator = (
          <div className="flex items-center gap-2 px-4 py-1.5" key={`sep-${index}`}>
            ...
            <span className="text-(length:--text-nano) font-medium text-muted-foreground uppercase tracking-wider">
              {t("activity.earlier", { defaultValue: "Earlier" })}
            </span>
            <div className="h-px flex-1 bg-border" />
          </div>
        );
  ...
  const renderGrouped = () => {
    ...
    return Array.from(groups.entries()).map(([groupKey, items]) => {
      ...
      const label = isOther
        ? t("activity.otherActivity", { defaultValue: "Other activity" })
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (agents.length === 0) {
      return {
        text: t("activity.noAgents", { defaultValue: "No agents set up yet. Add an agent to get started." }),
        showPulse: false,
      };
    }
    ...
    if (allPaused) {
      return {
        text: t("activity.allAgentsPaused", { defaultValue: "All agents are paused. Resume agents from the sidebar to see activity." }),
        showPulse: false,
      };
    }
    return {
      text: t("activity.agentsRunning", { defaultValue: "Your agents are running — activity will appear here shortly." }),
      showPulse: true,
    };
  }, [agents, t]);
    ...
    <aside className={cn("flex min-h-0 min-w-0 flex-1 flex-col bg-background", className)}>
      ...
      <div className="relative flex shrink-0 items-start justify-between gap-2 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold">{t("activity.agentFeed", { defaultValue: "Agent Feed" })}</h3>
          <p className="text-xs text-muted-foreground">
            {t("activity.liveFromAgents", { defaultValue: "Live activity from your agents" })}
          </p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                className="shrink-0 text-muted-foreground"
                    aria-label={t("activity.groupByTask", { defaultValue: "Group by task" })}
            ...
            <TooltipContent side="bottom">
              {groupMode === "flat"
                ? t("activity.groupByTask", { defaultValue: "Group by task" })
                : t("activity.showFlat", { defaultValue: "Show flat" })}
            </TooltipContent>
          </Tooltip>
          ...
          <DropdownMenu>
              ...
              <TooltipTrigger asChild>
                <DropdownMenuTrigger asChild>
                  <Button
                    ...
                    size="icon-sm"
                    className="shrink-0 text-muted-foreground"
                    aria-label={t("activity.filterBy", { defaultValue: "Filter by" })}
                  >
                    <ListFilter className="h-3.5 w-3.5" />
                  </Button>
                </DropdownMenuTrigger>
              </TooltipTrigger>
              <TooltipContent side="bottom">{t("activity.filterBy", { defaultValue: "Filter by" })}</TooltipContent>
            </Tooltip>
            <DropdownMenuContent align="end" className="w-48">
              ...
              >
                {FILTER_OPTIONS.map(({ value, key }) => (
                  <DropdownMenuRadioItem key={value} value={value}>
                    {t(`activity.filters.${key}`, { defaultValue: key })}
                  </DropdownMenuRadioItem>
                ))}
              </DropdownMenuRadioGroup>
              <DropdownMenuSeparator />
              ...
              >
                {t("activity.showAll", { defaultValue: "Show all activity" })}
              </DropdownMenuCheckboxItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center p-6 h-full">
            <div className="flex flex-col items-center gap-2 max-w-(--sz-16rem)">
              ...
              <p className="text-center text-sm text-muted-foreground">
                {emptyMessage?.text ?? t("activity.empty", { defaultValue: "Activity from your agents will appear here." })}
              </p>
            </div>
          </div>
        ) : groupMode === "flat" ? (
