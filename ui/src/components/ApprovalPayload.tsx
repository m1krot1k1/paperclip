...
export const typeLabel: Record<string, string> = {
  ...
  approve_ceo_strategy: i18n.t("approvals.type.approve_ceo_strategy"),
  budget_override_required: i18n.t("approvals.type.budget_override_required"),
  request_board_approval: i18n.t("approvals.type.request_board_approval"),
};
...
export function approvalLabel(type: string, payload?: Record<string, unknown> | null): string {
  const base = i18n.t(`approvals.type.${type}`, { defaultValue: type });
    ...
    <div className="mt-4 space-y-3.5 text-sm">
      {title && (
        <div className="space-y-1">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvals.payload.title")}</p>
          <p className="font-medium leading-6 text-foreground">{title}</p>
        </div>
      )}
      {summary && (
        <div className="space-y-1">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvals.payload.summary")}</p>
      ...
      {recommendedAction && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3.5 py-3">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-amber-700 dark:text-amber-300">
            {t("approvals.payload.recommendedAction")}
          </p>
      ...
      {nextActionOnApproval && (
        <div className="rounded-lg border border-border/60 bg-background/60 px-3.5 py-3">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvals.payload.onApproval")}</p>
          <p className="mt-1 leading-6 text-foreground">{nextActionOnApproval}</p>
        </div>
      )}
      {risks.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">{t("approvals.payload.risks")}</p>
      ...
      {proposedComment && (
        <div className="space-y-1.5">
          <p className="text-(length:--text-micro) font-medium uppercase tracking-(--tracking-label) text-muted-foreground">
            {t("approvals.payload.proposedComment")}
          </p>
