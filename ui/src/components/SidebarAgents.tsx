...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? t("agents.resume") : t("agents.pause");
  ...
  const pauseResumeDisabledLabel = disabled
    ...
    : isBudgetPaused
      ...
      : isPaused && hasInvalidOrgChain
        ? t("agents.invalidOrgChain")
      : pauseResumeLabel;
  const showBuiltInLifecycle = builtInStatus === "needs_setup" || builtInStatus === "pending_approval";
  const trailingLabel = [
    showBuiltInLifecycle ? t("agents.builtInStatus", { status: builtInStatus.replace(/_/g, " ") }) : null,
    hasInvalidOrgChain ? t("agents.invalidReportingChain") : null,
  ...
  const navItem = (
    <SidebarNavItem
      ...
      liveAccessory={
        agent.pauseReason === "budget" ? <BudgetSidebarMarker title={t("agents.pausedByBudget")} /> : undefined
      }
    />
  );
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("agents.openActions", { name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>{starred ? t("agents.removeFromStarred") : t("agents.star")}</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ) : null}
          ...
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("agents.pausedByBudgetLimits") : undefined}
          >
          ...
          >
            ...
            <span>{leaving ? t("agents.leaving") : t("agents.leave")}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
...
export function SidebarAgents({ streamlined = false }: { streamlined?: boolean } = {}) {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: t(action === "pause" ? "agents.pauseSuccess" : "agents.resumeSuccess"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: t(action === "pause" ? "agents.pauseError" : "agents.resumeError"),
  ...
  return (
    <SidebarSection
      ...
      menu={{
        ariaLabel: t("agents.sectionActions"),
        actions: [
          { type: "item", label: t("agents.browse"), icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: t("agents.sort"),
        radioChoices: [
          { value: "top", label: t("agents.sortTop") },
          { value: "alphabetical", label: t("agents.sortAlphabetical") },
          { value: "recent", label: t("agents.sortRecent") },
        ],
        radioValue: sortMode,
      ...
      {showSeeAllLink && (() => {
        ...
        const seeAllLink = (
          <Link
            ...
            aria-label={rail ? t("agents.seeAll") : undefined}
            onClick={() => {
              if (isMobile) setSidebarOpen(false);
            }}
          ...
          >
            ...
            <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : undefined}>{t("agents.seeAll")}</span>
          </Link>
        );
        return rail ? (
          <Tooltip>
            <TooltipTrigger asChild>{seeAllLink}</TooltipTrigger>
            <TooltipContent side="right">{t("agents.seeAll")}</TooltipContent>
          </Tooltip>
        ) : (
          seeAllLink
        );
