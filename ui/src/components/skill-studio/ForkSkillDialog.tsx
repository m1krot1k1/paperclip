...
export function ForkSkillDialog({
  ...
  const forkMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      toast?.pushToast({
        tone: "success",
        title: t("skills.editingCopy"),
        body:
          switched > 0
            ? t("skills.copyCreatedSwitched", {
                name: skill.name,
                count: switched,
                agentLabel: t(switched === 1 ? "skills.agent" : "skills.agents"),
              })
            : t("skills.copyCreated", { name: skill.name }),
      });
      onOpenChange(false);
    ...
    onError: (error) => {
      toast?.pushToast({
        tone: "error",
        title: t("skills.couldNotCreateCopy"),
        body: error instanceof Error ? error.message : t("skills.forkFailed"),
      });
    },
  });
  ...
  const forkLabel =
    reassign && agentCount > 0
      ? `${t("skills.createCopy")} & ${t("skills.switchAgents").toLowerCase()} ${agentCount} ${t(agentCount === 1 ? "skills.agent" : "skills.agents")}`
      : t("skills.createCopy");
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t("skills.editCopyTitle", { name: skill.name })}
          </DialogTitle>
          <DialogDescription>
            {t("skills.externalReadOnly", { name: skill.name })}
          </DialogDescription>
        </DialogHeader>

        {reusableFork ? (
          <div className="rounded-md border border-primary/40 bg-primary/5 p-3 text-sm">
            <p className="font-medium text-foreground">{t("skills.alreadyCopy")}</p>
            <p className="mt-0.5 text-muted-foreground">
              {t("skills.alreadyCopyDescription")}
            </p>
            <Button
              type="button"
              size="sm"
              ...
              <label className="mt-3 flex items-start justify-between gap-3">
                <span className="text-sm">
                  <span className="font-medium text-foreground">
                    {t("skills.switchAgents")}
                  </span>
                  <span className="mt-0.5 block text-xs text-muted-foreground">
                    {reassign
                      ? t("skills.switchAgentsOn")
                      : t("skills.switchAgentsOff")}
                  </span>
                </span>
                <ToggleSwitch
                  checked={reassign}
                  ...
                  disabled={busy}
                  aria-label={t("skills.switchAgents")}
                />
              </label>
            </>
          ) : (
            <p className="mt-1 text-xs text-muted-foreground">
              {t("skills.nothingAssigned")}
            </p>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          >
            ...
            {reusableFork ? t("skills.createAnotherCopy") : forkLabel}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
