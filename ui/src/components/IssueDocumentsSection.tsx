...
function getRevisionActor(
  ...
  if (revision.createdByUserId) {
    ...
    return {
      kind: "user",
      name: profile?.label ?? (revision.createdByUserId === "local-board" ? i18n.t("documents.board") : revision.createdByUserId.slice(0, 8)),
      imageUrl: profile?.image ?? null,
    };
  }
  return { kind: "system", name: i18n.t("documents.system") };
}
...
export function IssueDocumentsSection({
  ...
  const restoreDocumentRevision = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("documents.restoreFailed"));
    },
  });
  ...
  const setDocumentLock = useMutation({
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("documents.lockFailed"));
    },
  });
  ...
  const commitDraft = useCallback(async (
    ...
    if (!DOCUMENT_KEY_PATTERN.test(normalizedKey)) {
      setError(t("documents.keyInvalid"));
      if (options?.trackAutosave) {
        resetAutosaveState();
      }
      return false;
    ...
    try {
      ...
      if (isDocumentConflictError(err)) {
        try {
          ...
          setError(t("documents.remoteChanged"));
          return false;
        }
      }
      setError(err instanceof Error ? err.message : t("documents.saveFailed"));
      return false;
    }
  ...
  const previewRevision = useCallback((doc: IssueDocument, revisionId: string) => {
    ...
    if (documentConflict?.key === doc.key || documentHasUnsavedChanges(doc, draft)) {
      setError(t("documents.saveBeforeRevision"));
      return;
    }
    resetAutosaveState();
  ...
  const toggleDocumentLock = useCallback((doc: IssueDocument, locked: boolean) => {
    ...
    if (locked && (documentConflict?.key === doc.key || documentHasUnsavedChanges(doc, draft))) {
      setError(t("documents.saveBeforeLock"));
      return;
    }
      ...
      {isEmpty && !draft?.isNew ? (
        <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
          ...
          <Button variant="outline" size="sm" onClick={beginNewDocument} className="shrink-0">
            ...
            <span className="sm:hidden">{t("documents.newShort")}</span>
          </Button>
        </div>
      ) : (
      ...
      {draft?.isNew && (
        ...
        >
          <Input
            ...
            placeholder={t("documents.keyPlaceholder")}
          />
          {newDocumentKeyError && (
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            >
              {upsertDocument.isPending ? t("documents.saving") : t("documents.create")}
            </Button>
          </div>
        </div>
      )}
                ...
                actionsSlot={
                  <>
                    ...
                    <Button
                      ...
                      title={isLocked ? t("documents.unlock") : t("documents.lock")}
                      aria-label={isLocked ? t("documents.unlockNamed", { key: doc.key }) : t("documents.lockNamed", { key: doc.key })}
                    ...
                    >
                      ...
                      <span title={t("documents.locked")} aria-label={t("documents.locked")} className="inline-flex h-6 w-6 items-center justify-center text-amber-700 dark:text-amber-300">
                    ...
                    <Button
                      ...
                      title={copiedDocumentKey === doc.key ? t("documents.copied") : t("documents.copy")}
                    ...
                    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-3">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {t("documents.conflictDescription")}
                          </p>
                        </div>
                        <div className="flex flex-wrap items-center gap-2">
                          <Button
                      ...
                      {activeConflict.showRemote && (
                        <div className="mt-3 rounded-md border border-border/70 bg-background/60 p-3">
                          <div className="mb-2 flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
                            <span>{t("documents.remoteRevision", { revision: activeConflict.serverDocument.latestRevisionNumber })}</span>
                            <span>•</span>
