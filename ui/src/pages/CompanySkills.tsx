...
function sourceMeta(sourceBadge: CompanySkillSourceBadge, sourceLabel: string | null) {
  ...
  switch (sourceBadge) {
    case "skills_sh":
      return { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("skills.skillsShManaged") };
    case "github":
      return isSkillsShManaged
        ? { icon: VercelMark, label: sourceLabel ?? "skills.sh", managedLabel: t("skills.skillsShManaged") }
        : { icon: Github, label: sourceLabel ?? "GitHub", managedLabel: t("skills.githubManaged") };
    case "url":
      return { icon: Link2, label: sourceLabel ?? "URL", managedLabel: t("skills.urlManaged") };
    case "local":
      return { icon: Folder, label: sourceLabel ?? "Folder", managedLabel: t("skills.folderManaged") };
    case "paperclip":
      return { icon: Paperclip, label: sourceLabel ?? "Paperclip", managedLabel: t("skills.paperclipManaged") };
    default:
      return { icon: Boxes, label: sourceLabel ?? t("skills.catalog"), managedLabel: t("skills.catalogManaged") };
  }
}
...
function formatProjectScanSummary(result: CompanySkillProjectScanResult) {
  const parts = [
    t("skills.scanFound", { count: result.discovered }),
    t("skills.scanImported", { count: result.imported.length }),
    t("skills.scanUpdated", { count: result.updated.length }),
  ];
  if (result.conflicts.length > 0) parts.push(t("skills.scanConflicts", { count: result.conflicts.length }));
  if (result.skipped.length > 0) parts.push(t("skills.scanSkipped", { count: result.skipped.length }));
  return t(result.scannedWorkspaces === 1 ? "skills.scanSummary" : "skills.scanSummaryPlural", {
    summary: parts.join(", "),
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          title={activeFilterCount > 0 ? `${t("skills.filter")}: ${activeFilterCount}` : t("skills.filter")}
        >
...
function ProvenanceBadge({ packageName, packageVersion }: { packageName: string | null; packageVersion: string | null }) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{t("skills.catalogProvenanceTooltip")}</TooltipContent>
    </Tooltip>
  );
}
...
function skillSettingsToastBody(skill: Pick<CompanySkillDetail, "categories" | "sharingScope">) {
  const sharing = t(skill.sharingScope === "private" ? "skills.sharingPrivate" : "skills.sharingCompany");
  const categories = skill.categories.length
    ? t("skills.categoriesValue", { categories: skill.categories.join(", ") })
    : t("skills.categoriesNone");
    ...
    >
      <div className="flex items-start gap-3">
        {selectMode && canMove ? (
          <input
            ...
            checked={selected}
            aria-label={t("skills.selectNamed", { name: card.name })}
      ...
      <div className="mt-auto pt-3">
        ...
        <div className="flex items-center gap-2 text-(length:--text-micro) text-muted-foreground">
          <span>{t(card.agentCount === 1 ? "skills.agentCount" : "skills.agentsCount", { count: card.agentCount })}</span>
          {card.starCount > 0 ? (
            <>
    ...
    <div className="flex min-h-(--sz-calc-30) md:h-(--sz-calc-33) md:min-h-0 md:overflow-hidden">
      ...
      <aside className={cn("hidden w-60 shrink-0 flex-col overflow-hidden border-r border-border md:flex", showFolderRail && "md:hidden")}>
        <div className="border-b border-border px-4 py-4">
          <h2 className="text-sm font-semibold text-foreground">{t("skills.skillsStore")}</h2>
                ...
                message={
                  totalCount === 0
                    ...
                    : search || activeCategory || sourceFilterActive
                      ...
                      : t("skills.noTabSkills")
                }
              />
              {totalCount === 0 ? (
                <div className="mt-3 flex flex-col items-center gap-2">
                  <Button size="sm" onClick={onBrowseCatalog}>
                    <Boxes className="mr-1.5 h-3.5 w-3.5" /> {t("skills.browseCatalogAction")}
                  </Button>
                  <Button size="sm" variant="ghost" onClick={onCreate}>
                    {t("skills.createSkillAction")}
                  </Button>
                </div>
              ) : (search || activeCategory || sourceFilterActive) ? (
    ...
    <div className="space-y-4">
      ...
      {draft.forkedFromName ? (
        <div className="flex items-center gap-2 rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
          ...
          {t("skills.forkingSkill", { name: draft.forkedFromName })}
        </div>
      ) : null}
      ...
      {step === 0 ? (
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              placeholder={t("skills.categoriesPlaceholder")}
              className="h-9"
            />
          </div>
        </div>
        ...
        <div className="space-y-4 text-sm">
          <div className="grid grid-cols-(--gtc-26) gap-y-2">
            ...
            <span className="text-muted-foreground">{t("skills.scope")}</span>
              ...
              {(["company", "private"] as const).map((scope) => (
                ...
                >
                  ...
                  <span className="mt-1 block text-xs text-muted-foreground">
                    {scope === "company" ? t("skills.companyVisible") : t("skills.privateVisible")}
                  </span>
                </button>
              ))}
              <button
  ...
  if (skill.compatibility === "invalid") {
    cta = (
      <Tooltip>
        ...
        <TooltipContent>{t("skills.invalidInstallTooltip")}</TooltipContent>
      </Tooltip>
    );
  } else if (!isInstalled) {
    cta = (
      <Button onClick={onInstall} disabled={loadingPrimaryAction}>
        ...
        {loadingPrimaryAction ? t("skills.preparing") : t("skills.installInOrganization")}
      </Button>
    );
    ...
    <div className="min-w-0">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-4">
          ...
          {hashOutOfSync ? (
            <Tooltip>
              ...
              <TooltipContent>{t("skills.catalogHashChangedTooltip")}</TooltipContent>
            </Tooltip>
          ) : null}
          {skill.requires.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("skills.requiresLabel")}: {skill.requires.join(", ")}
            </Badge>
          ) : null}
          {skill.recommendedForRoles.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("skills.rolesLabel")}: {skill.recommendedForRoles.join(" · ")}
            </Badge>
          ) : null}
          {skill.tags.length > 0 ? (
            <Badge variant="outline" className="border-border bg-muted/40 text-(length:--text-micro) text-muted-foreground">
              {t("skills.tagsLabel")}: {skill.tags.join(" · ")}
            </Badge>
          ) : null}
        </div>

        <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          <span className="uppercase tracking-(--tracking-caps)">{t("skills.key")}</span>
          <span className="font-mono">{skill.key}</span>
          <span className="uppercase tracking-(--tracking-caps)">·</span>
          <span className="uppercase tracking-(--tracking-caps)">{t("skills.hash")}</span>
          <span className="font-mono">{skill.contentHash.slice(0, 24)}…</span>
          <CopyText
            text={skill.contentHash}
            copiedLabel={t("skills.copiedHash")}
            ariaLabel={t("skills.copyContentHash")}
            title={t("skills.copyContentHash")}
      ...
      <div className="min-h-(--sz-400px) px-5 py-5">
        {fileQuery.isLoading ? (
          ...
          <div className="text-sm text-destructive">{fileQuery.error instanceof Error ? fileQuery.error.message : t("skills.failedToLoadFile")}</div>
        ) : !fileQuery.data ? (
...
function InstallPreviewDialog({
  ...
  if (defaultAction === "update") {
    ...
    confirmLabel = t("skills.replaceExistingSkill");
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => (!isPending ? onOpenChange(value) : null)}>
      <DialogContent className="sm:max-w-2xl" showCloseButton={!isPending}>
        <DialogHeader>
          <DialogTitle>
            {defaultAction === "update" ? t("skills.update") : defaultAction === "replace" ? t("skills.replace") : t("skills.install")} · {skill.name}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 text-sm">
          <div className="rounded-md border border-border p-3">
            <div className="grid grid-cols-(--gtc-26) gap-y-2 text-xs">
              ...
              <div className="text-foreground">{skill.recommendedForRoles.length === 0 ? t("skills.any") : skill.recommendedForRoles.join(" · ")}</div>
          ...
          {advancedOpen ? (
            <div className="space-y-3 rounded-md border border-border p-3 text-xs">
              <div>
                <label className="mb-1 block uppercase tracking-wide text-muted-foreground">{t("skills.slugOverride")}</label>
              ...
              <label className="flex items-center gap-2">
                ...
                <span>{t("skills.forceReplaceSameKey")}</span>
              </label>
            </div>
          ) : null}
...
function AttachAgentsPopover({
  ...
  return (
    <AgentMultiSelect
      ...
      pending={pending}
      triggerLabel={t("skills.addToAgent")}
      ...
      emptyMessage={eligible.length === 0 ? t("skills.noSkillCapableAgents") : t("skills.noAgentsShort")}
...
const SKILL_DETAIL_TABS: Array<{ value: SkillDetailTab; label: string; icon: typeof FileText }> = [
  { value: "overview", label: t("skills.overview"), icon: FileText },
  { value: "files", label: t("skills.files"), icon: FolderOpen },
  { value: "versions", label: t("skills.versions"), icon: History },
  { value: "agents", label: t("skills.agents"), icon: Users },
];
      ...
      <DialogContent className="flex max-h-(--sz-85vh) w-full !max-w-(--pct-90) flex-col overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            ...
            <label className="flex items-center gap-2">
              <Badge variant="outline" className="border-green-500/30 bg-green-500/10 uppercase tracking-wider text-green-400">{t("skills.newVersion")}</Badge>
              <select
                value={right?.id ?? ""}
        ...
        <div className="flex min-h-0 flex-1 gap-3">
          ...
          <div className="min-w-0 flex-1 overflow-auto rounded-md border border-border text-xs">
            {!right ? (
              <div className="p-6 text-center text-sm text-muted-foreground">{t("skills.selectVersionToCompare")}</div>
            ) : left?.id === right.id ? (
    ...
    <section>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        >
          ...
          {copied ? t("skills.copiedPath") : t("skills.copyPath")}
        </Button>
        {onMove ? (
      ...
      <div className="flex flex-wrap gap-1.5">
        {categories.map((tag) => (
          ...
          >
            ...
            <button
              type="button"
              aria-label={t("skills.removeTag", { tag })}
              disabled={pending}
            ...
            <div className="flex items-center gap-2">
              ...
              {skill.editable && file?.editable ? (
                ...
                <Button
                  ...
                  size="sm"
                  onClick={onFork}
                  title={skill.editableReason ?? t("skills.forkThis")}
                >
      ...
      <div className="space-y-6">
        ...
        <section className="grid min-w-0 gap-3 text-sm sm:grid-cols-2">
          <div className="min-w-0 border-b border-border py-2">
            <div className="text-xs text-muted-foreground">{t("skills.key")}</div>
          ...
          <div className="min-w-0 border-b border-border py-2">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              {skill.editable ? (
                ...
                <>
                  <span>{t("skills.readOnlyMode")}</span>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">
            {versionsLoading ? t("skills.loadingSkills") : t(versions.length === 1 ? "skills.versionCount" : "skills.versionsCount", { count: versions.length })}
          </div>
          <Button
            type="button"
            variant="outline"
          ...
          >
            <History className="mr-1.5 h-3.5 w-3.5" /> {t("skills.compare")}
          </Button>
        </div>
        <div className="border-y border-border">
          {versionsLoading ? (
            ...
            <div className="py-6 text-sm text-muted-foreground">{t("skills.noVersions")}</div>
          ) : (
            sortedVersions.map((version) => (
    ...
    <div className="min-h-(--sz-calc-30)">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span
                        ...
                        aria-label={t("skills.installedFrom", { source: source.label })}
                      >
                        <SourceIcon className="h-4 w-4" />
                      </span>
                    </TooltipTrigger>
                    <TooltipContent>{t("skills.installedFrom", { source: source.label })}</TooltipContent>
                  </Tooltip>
                </div>
                ...
                {subtitleText ? (
                  <div className="mt-1 max-w-2xl">
                    ...
                    {descClamped ? (
                      ...
                      >
                        {descExpanded ? t("common.showLess") : t("common.viewAll")}
                      </button>
                    ) : null}
                  </div>
                ) : null}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-muted-foreground">
                    ...
                    <span className="hidden sm:inline">{detail.attachedAgentCount === 1 ? t("skills.install") : t("skills.installs")}</span>
                  </span>
                </TooltipTrigger>
                <TooltipContent>{t("skills.agentsInstalled")}</TooltipContent>
              </Tooltip>
              <button
                type="button"
                ...
                title={detail.starredByCurrentActor ? t("skills.unstar") : t("skills.starSkill")}
              >
                <Star className={cn("h-3.5 w-3.5", detail.starredByCurrentActor && "fill-current text-yellow-400")} />
                <span className="hidden sm:inline">{detail.starredByCurrentActor ? t("skills.starred") : t("skills.star")}</span>
      ...
      <div className="grid gap-6 px-4 py-4 xl:grid-cols-(--gtc-29)">
        ...
        <aside className="min-w-0 space-y-6 border-t border-border pt-4 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0">
                ...
                <div className="space-y-0.5">
                  ...
                  {detail.usedByAgents.slice(0, 3).map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {meta?.paused ? (
                          <Pause className="h-3 w-3 shrink-0 text-amber-500" aria-label={t("skills.paused")} />
                        ) : null}
                      </Link>
                    );
                  })}
                  {detail.usedByAgents.length > 3 ? (
                    <p className="px-1.5 pt-0.5 text-xs text-muted-foreground">
                      {t("skills.more", { count: detail.usedByAgents.length - 3 })}
                    </p>
                  ) : null}
                </div>
              )}
          ...
          {detail.sourceType === "github" ? (
            <section>
              <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">{t("skills.updates")}</div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Pin className="h-3.5 w-3.5 shrink-0" aria-label={t("skills.pinnedSourceRevision")} />
                    </TooltipTrigger>
                  <TooltipContent>{t("skills.pinnedSourceRevision")}</TooltipContent>
                  </Tooltip>
      ...
      {isDirty ? (
        <div className="fixed bottom-6 left-1/2 z-40 flex -translate-x-1/2 items-center gap-3 rounded-full border border-border bg-background/95 px-4 py-2 shadow-lg backdrop-blur">
          ...
          <Button size="sm" onClick={onSave} disabled={savePending}>
            <Save className="mr-1.5 h-3.5 w-3.5" />
            {savePending ? t("skills.saving") : t("skills.saveFileChanges")}
          </Button>
        </div>
      ) : null}
      ...
      <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("skills.skillSettings")}</DialogTitle>
            <DialogDescription>{t("skills.manageGroupingSharing", { name: detail.name })}</DialogDescription>
          </DialogHeader>
          <div className="space-y-5">
            <div className="space-y-1.5">
              ...
              />
              <p className="text-xs text-muted-foreground">{t("skills.categoriesHint")}</p>
            </div>
            <div className="space-y-1.5">
              ...
              >
                <option value="company">{t("skills.companyVisibleOption")}</option>
                <option value="private">{t("skills.privateVisibleOption")}</option>
              </select>
              <p className="text-xs text-muted-foreground">{t("skills.publicLinkComingLater")}</p>
            </div>
            <div className="flex justify-end gap-2 border-t border-border pt-4">
              ...
              >
                <Save className="mr-1.5 h-3.5 w-3.5" />
                {updateSettingsPending ? t("skills.saving") : t("skills.saveSettings")}
              </Button>
            </div>
            {detail.editable ? (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                ...
                {detail.sourcePath && displaySourcePath ? (
                  <>
                    ...
                    <CopyText
                      ...
                      ariaLabel={t("skills.copySourcePath")}
                      title={t("skills.copySourcePath")}
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{detail.editable ? t("skills.editable") : t("skills.readOnlyMode")}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {readonlyMetadataValue(detail.metadata, "userModifiedAt") ? (
              <Tooltip>
                ...
                <TooltipContent>{t("skills.editedAfterInstallTooltip")}</TooltipContent>
              </Tooltip>
            ) : null}
            {(() => {
          ...
          <div className="flex flex-wrap items-start gap-x-3 gap-y-2">
            ...
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("skills.noAgentsAttachedShort")}</span>
            ) : (
...
export function CompanySkills() {
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: t(result.imported.length === 1 ? "skills.skillAdded" : "skills.skillsAdded", { count: result.imported.length }),
      });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: () => {
      setScanStatusMessage(t("skills.scanningWorkspaces"));
    },
    onSuccess: async (result) => {
      setScanStatusMessage(t("skills.scanRefreshing"));
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        ...
        body: t("skills.skillEditableInWorkspace", { name: skill.name }),
      });
    },
    onError: (error) => {
  ...
  const updateSkillSettings = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({ tone: "success", title: t("skills.skillSettingsUpdated"), body: skillSettingsToastBody(skill) });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("skills.skillSettingsUpdateFailed"),
  ...
  async function handleAttachSubmit(nextAgentIds: string[], versionId: string | null = null) {
    ...
    try {
      ...
      pushToast({ tone: "success", title: t("skills.agentsUpdated"), body: t("skills.agentsAttached", { count: nextAgentIds.length }) });
    } catch (error) {
      pushToast({ tone: "error", title: t("skills.updateFailed"), body: error instanceof Error ? error.message : t("skills.agentSkillsUpdateFailed") });
    }
  }
  ...
  const deleteSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        ...
        body: t("skills.skillRemovedBody", { name: skill.name }),
      });
    },
    onError: (error) => {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Boxes} message={t("skills.selectCompanyManage")} />;
  }
  ...
  // "{t("skills.backToStore")}" returns to the discovery grid while keeping the tab /
  ...
  return (
    <>
      ...
      <Dialog open={deleteOpen} onOpenChange={closeDeleteDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("skills.removeSkillDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <p>
              {deleteTargetDetail
                ? t("skills.aboutToRemoveNamed", { name: deleteTargetDetail.name })
                : t("skills.aboutToRemove")}
            </p>
            {deleteTargetDetail?.usedByAgents?.length ? (
              <div className="rounded-md border border-border px-3 py-3 text-muted-foreground">
                {t("skills.currentlyUsedBy", { agents: deleteTargetDetail.usedByAgents.map((agent) => agent.name).join(", ") })}
              </div>
            ) : null}
            {(deleteTargetDetail?.usedByAgents.length ?? 0) > 0 ? (
              <p className="text-muted-foreground">
                {t("skills.detachToRemove")}
              </p>
            ) : null}
          </div>
          <DialogFooter>
      ...
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("skills.addSkillSourceDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            <a
            ...
            >
              <span>
                ...
                <span className="mt-1 block text-muted-foreground">
                  {t("skills.findSkillRepositories")}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("skills.importSkillTitle")}</DialogTitle>
            <DialogDescription>
              {t("skills.importSkillDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="flex items-center gap-2 border-b border-border pb-2">
              ...
              <Button size="sm" onClick={handleAddSkillSource} disabled={importSkill.isPending}>
                {importSkill.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : t("skills.importSkill")}
              </Button>
            </div>
            <a
              href="https://skills.sh"
            ...
            >
              <span>
                <span className="block font-medium">{t("skills.browseSkillsSh")}</span>
                <span className="mt-1 block text-muted-foreground">{t("skills.findInstallCommands")}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
            <a
            ...
            >
              <span>
                <span className="block font-medium">{t("skills.searchGitHub")}</span>
                <span className="mt-1 block text-muted-foreground">{t("skills.findSkillRepositoriesShort")}</span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
          </div>
      ...
      <MobileFolderSheet
        ...
        selection={folderSelection}
        allLabel={t("skills.allSkills")}
        itemLabelPlural={t("skills.skills")}
      ...
      {isStudioNew ? (
        ...
        <DiscoveryGrid
          ...
          onOpenMoveCard={showInstalledFolders ? (card) => {
            ...
            setMoveDialog({
              ...
              title: t("skills.moveNamed", { name: card.name }),
              subtitle: t("skills.chooseDestinationFolder"),
          ...
          onMoveCard={showInstalledFolders ? (card, folderId) => {
            ...
            pushToast({
              tone: "success",
              title: t("skills.skillMoved"),
              body: folderId
                ? t("skills.movedToFolder", { name: card.name, folder: skillFolderResult?.folders.find((folder) => folder.id === folderId)?.name ?? t("skills.folder") })
                : t("skills.movedToUnfiled", { name: card.name }),
              action: {
                label: t("skills.undo"),
        ...
        <SkillDetailPage
          ...
          onMoveToFolder={activeDetail ? () => setMoveDialog({
            ...
            title: t("skills.moveNamed", { name: activeDetail.name }),
            subtitle: t("skills.chooseDestinationFolder"),
