...
export function ProfileSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("settings.settings"), href: "/company/settings" },
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("activity.board");
  }
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.failedToUpdateProfile"));
    },
  });
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("settings.selectCompanyBeforeAvatar"));
      }
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.failedToUploadAvatar"));
    },
  });
  ...
  const removeAvatarMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.failedToRemoveAvatar"));
    },
  });

  if (sessionQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("settings.loadingProfile")}</div>;
  }

  if (sessionQuery.error || !sessionQuery.data) {
    return (
      <div className="text-sm text-destructive">
        {sessionQuery.error instanceof Error ? sessionQuery.error.message : t("settings.failedToLoadProfile")}
      </div>
    );
  }

  const currentName = name.trim() || sessionQuery.data.user.name || t("activity.board");
  ...
  const uploadHint = selectedCompany
    ? t("settings.storedInFileStorage", { company: selectedCompany.name })
    : t("settings.selectCompanyForAvatar");
      ...
      <section className="space-y-8">
        <Card className="block relative overflow-hidden rounded-(--rad-28) border-border/70">
          ...
          <div className="relative p-6 pt-10">
            <div className="flex flex-wrap items-end gap-5 rounded-(--rad-24) border border-border/70 bg-background/92 p-5 shadow-(--shadow-extract-18) backdrop-blur-sm">
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {uploadAvatarMutation.isPending ? <LoaderCircle className="size-4 animate-spin" /> : <Camera className="size-4" />}
                    {currentImage ? t("settings.changePhoto") : t("settings.uploadPhoto")}
                  </Button>
                  {currentImage ? (
                    <Button
                      type="button"
            ...
            <p className="truncate text-sm text-muted-foreground">{sessionQuery.data.user.email ?? t("settings.noEmail")}</p>
                </div>
                <p className="max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("settings.avatarDescription", { hint: uploadHint })}
                </p>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div className="md:col-span-2 flex justify-end">
            <Button type="submit" disabled={isSavingProfile || !name.trim()}>
              ...
              {updateMutation.isPending ? t("settings.saving") : t("settings.saveProfile")}
            </Button>
          </div>
        </form>
